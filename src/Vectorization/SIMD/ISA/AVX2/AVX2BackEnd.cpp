...
SIMDBackEnd::SIMDInstListType AVX2BackEnd::vgather(VOperand V) {
  ...
  auto CopyIdx = std::vector<long>(V.Idx.begin(), V.Idx.begin() + V.Idx.size());
  ...
  for (size_t T = LeftOver; T < V.Size; ++T) {
    VIndex += std::to_string(CopyIdx[T-LeftOver]) + ((T == (V.Size - 1)) ? "" : ", ");
  }
  VIndex += ")";

  // Memory address
  if( BaseIdx == "+" ) {
	  MACVETH_DEBUG( "Gabriel", "This needs to be fixed" );
	  // Essentially, what is going on here is that (I think) this node has been generated by a previous reduction?
	  BaseIdx = V.UOP[MinIdx-CopyIdx.begin()]->getOutputInfoName();
  }
