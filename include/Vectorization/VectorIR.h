...
public:
  ...
  static inline std::vector<std::tuple<std::vector<int>, std::string>> MapLoads;
...
struct VOperand {
  ...
  bool checkIfVectorAssigned(int VL, NodeVectorT &V, MVDataType::VWidth W);
  /// Get width
  ...
  bool checkIfAlreadyStored(NodeVectorT &V);

  /// Generate a name according to the VID
  std::string genNewVOpName() {
    return VectorIR::VOP_PREFIX + std::to_string(VID++);
  }
...
struct VectorOP {
  /// Type of operation
  VectorIR::VType VT = VectorIR::VType::MAP;
  ...
  void setResult(const VOperand &R) { this->R = R; }
  ...
  /// Check if operation is sequential or not
  bool isReduction() { return VT == VectorIR::VType::REDUCE; }
