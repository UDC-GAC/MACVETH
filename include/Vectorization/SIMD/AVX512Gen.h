 ...
 * Last Modified Date: Ven 02 Out 2020 12:52:29 CET
 * Last Modified By  : Marcos Horro <marcos.horro@udc.gal>
 */

#ifndef MACVETH_AVX512GEN_H
#define MACVETH_AVX512GEN_H
...
/// SIMD generator for AVX512 architectures
class AVX512Gen : public SIMDGenerator {
public:
  /// Name of the architecture
  static inline std::string NArch = "x86";
  /// Name of the ISA
  static inline std::string NISA = "AVX512";
  ...
  /// One of the optimizations included in AVX512
  ...
  virtual std::string getNArch() override { return AVX512Gen::NArch; }

  /// Get name of AVX architecture
  virtual std::string getNISA() override { return AVX512Gen::NISA; }
  ...
  virtual std::string getMapWidth(VectorIR::VWidth V) override {
    if (V <= 128) {
      ...
      return MapWidth[VectorIR::VWidth::W512];
    }
  }

  /// Get the translation between VectorIR data types and AVX512's
  virtual std::string getMapType(VectorIR::VDataType D) override {
    return MapType[D];
  }

  /// Get max width
  virtual int getMaxWidth() override { return AVX512Gen::MaxWidth; }
  ...
  static SIMDGenerator *getSingleton() {
    if (AVX512Gen::_instance == 0) {
      AVX512Gen::_instance = new AVX512Gen();
    }
    return AVX512Gen::_instance;
  };

private:
  /// Constructor
  AVX512Gen() : SIMDGenerator() { SIMDGenerator::populateTable(MVISA::AVX512); }
  ...
  createSIMDInst(std::string Op, std::string Res, std::string Width,
                 std::string DataType, std::string PrefS, std::string SuffS,
                 std::list<std::string> Args, SIMDGenerator::SIMDType SType,
                 int TacID);
  ...
  /// Get mask register according to a mask, number of elements and vector width
  ...
  /// Mapping the width types with its name in AVX512
  static inline std::map<VectorIR::VWidth, std::string> MapWidth = {
      {VectorIR::VWidth::W128, ""},
      {VectorIR::VWidth::W256, "256"},
      {VectorIR::VWidth::W512, "512"}};

  /// Map of VectorIR types and its translation in the AVX512 architecture
