...
if [ $# -ne 2 ]; then
    echo "Usage: ./time_benchmarh.sh <binary_name> <FLOPS>";
...
compute_mean_exec_time()
{
    ...
    PROCESSED_TIME="$time";
    FLOPS=`echo "scale=10; $flops/$time" | bc`;
    PROCESSED_GFLOPS=`echo "scale=10; $FLOPS/1000000000" | bc`;
    rm -f avg.out;
}
...
TH=OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0

OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0 $1 > ____tempfile.data.polybench;
OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0 $1 >> ____tempfile.data.polybench;
OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0 $1 >> ____tempfile.data.polybench;
OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0 $1 >> ____tempfile.data.polybench;
OMP_NUM_THREADS=1 GOMP_CPU_AFFINITY=0 $1 >> ____tempfile.data.polybench;

compute_mean_exec_time "____tempfile.data.polybench" "$1" "$2";
echo "[INFO] Normalized time: $PROCESSED_TIME";
echo "[INFO] Normalized GFLOPS/s: $PROCESSED_GFLOPS";
