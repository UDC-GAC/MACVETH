...
#pragma macveth
    for (t = 0; t < TSTEPS; ++t) {
        for (i = 0; i < _PB_N; i += 4) {
            __m256d ymm0, ymm1, ymm2, ymm3, ymm4, vop0, vop1;
            vop1 = _mm256_load_pd(&x[i + 0]);                // cost = 1
            ...
            ymm4 = _mm256_set_pd(G, G, G, G);                // cost = 1
            vop0 = _mm256_add_pd(ymm4, ymm3);                // cost = 1
            _mm_store_sd(&G, _mm256_castpd256_pd128(vop0));
            //_mm256_maskstore_pd(&G, _mm256_set_epi64x(0, 0, 0, 0xffffffffffffffff),
            //        vop0);                       // cost = 1
            vop1 = _mm256_load_pd(&y[i + 0]);                // cost = 1
            ymm0 = _mm256_permute_pd(vop1, 0x05);            // cost = 1
            ymm1 = _mm256_add_pd(vop1, ymm0);                // cost = 1
            ymm2 = _mm256_permute2f128_pd(ymm1, ymm1, 0x01); // cost = 1
            ymm3 = _mm256_add_pd(ymm2, ymm1);                // cost = 1
            ymm4 = _mm256_set_pd(H, H, H, H);                // cost = 1
            vop0 = _mm256_add_pd(ymm4, ymm3);                // cost = 1
            _mm_store_sd(&H, _mm256_castpd256_pd128(vop0));
            //_mm256_maskstore_pd(&H, _mm256_set_epi64x(0, 0, 0, 0xffffffffffffffff),
            //                    vop0); // cost = 1
            // statement replaced: H = H + y[i];
        }
    }
#pragma endmacveth
    (*S) = G;
