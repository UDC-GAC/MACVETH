 ...
 * Last Modified Date: Lun 09 Dec 2019 14:39:58 MST
...
inline void lrn(quote_vector3D(bottom), float k, float alpha, float beta,
  ...
  for (int h = 0; h < height; ++h) {
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < channel; ++c) {
        int c_start =
            std::max(0, c - local_size / 2); //确定 lrn计算时取的相邻channel范围
...
inline void convolution1(quote_vector3D(bottom), int stride, int pad,
  ...
  int hout = (height - kernel_size) / stride +
...
#pragma omp parallel for
    for (int h = 0; h < hout; ++h) //计算输出的一个点 (h,w)
      for (int w = 0; w < wout; ++w) {
        ...
        int w_start = w * stride;
        int w_end = w_start + kernel_size; //求cin x kernel_size x
