...
void kernel_spmv( double * restrict A,
                  double * restrict x,
	          double * restrict y,
                  long long n,
                  long long * indptr,
...
void init_array(char * spfpath,
    ...
    // Read NNZ, incorporated points, shapes in dictionary, matrix shape (4 x integer)
    ...
    *nrow = intnrow;
    ...
    for( int i = 0; i < nshapes; ++i ) {
        ...
        if( debug ) printf( "\t\tASTs start at offset: %d\n", ast_pointer );
    }
    ...
    for( int i = 0; i < nshapes; ++i ) {
        ...
        if( debug ) printf( "\tNumber of ASTs: %d\n", nASTs );

        // Read pointer to data start
        int data_pointer;
        read( fd, &data_pointer, sizeof(int) );
        if( debug ) printf( "\tData starts at offset: %d\n", data_pointer );

        if( debug ) printf( "\tLocations:\n" );
        for( int j = 0; j < nASTs; ++j ) {
            int loc[2];
            read( fd, loc, 2*sizeof(int) );
            if( debug ) printf( "\t\tAST #%d: ( %d, %d )\n", j, loc[0], loc[1] );
        }
    }
