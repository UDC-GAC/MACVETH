...
inline void fullconnection1(const quote_vector3D(bottom), const quote_vector4D(weights), const vector <float> &bias, vector <float> & top){
...
inline void fullconnection2(const vector <float>& bottom , const vector < vector <float> >& weights, const vector <float> &bias, vector <float> & top){
	int channel = bottom.size();
	int num_output = top.size();
	//top.assign(num_output, 0);
	//	#pragma clang loop critical(disable)
...
inline void fullconnection3(const vector <float>& bottom , const vector < vector <float> >& weights, const vector <float> &bias, vector <float> & top){
	int channel = bottom.size();
	int num_output = top.size();
	//top.assign(num_output, 0);
	//	#pragma clang loop critical(disable)
...
//inline void fullconnection(const vector <float>& bottom , const vector < vector <float> >& weights, vector <float> & top){
//  printf("fc3\n");
//	int channel = bottom.size();
//	int num_output = top.size();
//	top.assign(num_output, 0);
...
inline void lrn(quote_vector3D(bottom), float k, float alpha, float beta, int local_size){
	...
	for(int h = 0; h < height; ++h){
		for(int w = 0; w < width; ++w){
			for(int c = 0; c < channel; ++c){
				int c_start = std::max(0, c-local_size/2);  //确定 lrn计算时取的相邻channel范围
				...
				temp_vec[c] = bottom[c][h][w] / powf(temp, beta); //pow 为幂运算
			}
			for(int c = 0; c < channel; ++c){
...
inline void pooling( quote_vector3D (bottom),int kernel_size, int stride, int pad, quote_vector3D(top), string type){
  ...
  //double power_sw = 1.32;
	...
	if(pad){  //如果要加边框的话
		add_pad(bottom,pad);  //我们这里定义add_pad(blob bottom, int pad)函数，给每一个图像加上一个厚度为pad，像素值为0的边框
		//则此时 bottom：n x c x (h+2*pad) x (w+2*pad)
	}
	int channel = bottom.size();
	int height = bottom[0].size();
	int width = bottom[0][0].size();
	int hout = (height - kernel_size)/stride + 1; //根据输入的大小，以及pooling模板大小和步长可以计算出pooling输出的大小
	...
	for(int c = 0; c < channel;  ++c)        //对于一张图片产生的每一个channel
		for(int h = 0; h < hout; ++h)        //计算输出的一个点 (h,w)
			for(int w = 0; w < wout; ++w){
				...
				int h_end = h_start + kernel_size;  //这里h_end和w_end应该是不会出界的
...
inline void convolution1( quote_vector3D (bottom), int stride, int pad, const quote_vector4D(weights), const vector <float>& bias, quote_vector3D (top), string layer_name){
	if(pad){  //如果要加边框的话
		add_pad( bottom,pad);  
	}
	int channel = bottom.size();
	...
	int hout = (height - kernel_size)/stride + 1; //根据输入的大小，以及卷积模板大小和步长可以计算出卷积输出的大小
	int wout = (width - kernel_size)/stride + 1;
	// #pragma clang loop critical(disable)
	#pragma omp parallel for
	for (int co = 0; co < cout; ++co){        //一个图片产生的每一个输出的channel
		...
		for (int w = 0; w < wout; ++w){
			...
			int w_start = w*stride;
			int w_end = w_start + kernel_size; //求cin x kernel_size x kernel_size个数据对应相乘的和；
			float sum = 0;
...
inline void convolution2( quote_vector3D (bottom), int stride, int pad, const quote_vector4D(weights), const vector <float>& bias, quote_vector3D (top), string layer_name){
	if(pad){  //如果要加边框的话
		add_pad( bottom,pad);  
	}
	int channel = bottom.size();
...
#pragma omp parallel for
	...
	gettimeofday(&tend,NULL);
	timediff = ((tend.tv_sec - tstart.tv_sec)*1000000.00) + (tend.tv_usec - tstart.tv_usec);
	//timediff = (tdiff.tv_sec*1000000.00) + tdiff.tv_usec;
	printf("Total Time (Enc + Dec): %f ms \n", timediff/1000.00);
}


inline void convolution3( quote_vector3D (bottom), int stride, int pad, const quote_vector4D(weights), const vector <float>& bias, quote_vector3D (top), string layer_name){
	if(pad){  //如果要加边框的话
		add_pad( bottom,pad);  
	}
	int channel = bottom.size();
...
inline void convolution4( quote_vector3D (bottom), int stride, int pad, const quote_vector4D(weights), const vector <float>& bias, quote_vector3D (top), string layer_name){
	if(pad){  //如果要加边框的话
		add_pad( bottom,pad);  
	}
	int channel = bottom.size();
...
inline void convolution5( quote_vector3D (bottom), int stride, int pad, const quote_vector4D(weights), const vector <float>& bias, quote_vector3D (top), string layer_name){
	if(pad){  //如果要加边框的话
		add_pad( bottom,pad);  
	}
	int channel = bottom.size();
...
inline void readweights(string ss, quote_vector4D(weights), vector <float>& bias){
	FILE *fin;
	fin = fopen(ss.c_str(), "rb");
...
inline void readweights(string ss, vector < vector <float> >& weights, vector <float>& bias){
...
*/
inline void EmbedLayer(int word_input, const vector <vector <float> > & embed_weights, vector <float> & input_sentence){
...
inline void PredictLayer(const vector <float>& bottom , const vector < vector <float> >& weights, const vector <float> &bias, vector <float> & top){
...
inline void LSTMFullconnectionLayer(const vector <float>& bottom , const vector < vector <float> >& weights, vector <float> & top){
	//lstm中的全连接计算
	int channel = bottom.size();
	int num_output = top.size();
	top.assign(num_output, 0);
...
inline void Add_Result( vector <float>& gate_input_t, const vector <float>& xcstatic_rst, const vector <float>& Wxc_tm1, const vector <float>& Whc_tm1, const vector <float>& xc_bias){
	//全连接计算结果求和
...
//}
inline void LSTMlayer(int cont_input, const vector <float>& gate_input_t, vector <float>& c_tm1, vector <float>& hc_tm1){
	//非线性操作更新ht和ct
	int length = gate_input_t.size();
	float it, ft, ot, gt;
...
int main(){
	...
	vector4D(conv1_weights, num_output, channels, conv_kernel_size, conv_kernel_size); 
	vector <float> conv1_bias(num_output);
	readweights("conv1.bin", conv1_weights, conv1_bias);
	int stride = 4, pad = 0; 
	vector3D(conv1_rst, num_output, 55, 55);  
	//compute
	convolution1(image, stride, pad, conv1_weights, conv1_bias,  conv1_rst, "conv1");
	//free space
	release3D(image);
	release (conv1_bias);
...
//	lrn(conv1_rst, k, alpha, beta, local_size);
	...
	copy(pool1_rst.begin(),pool1_rst.begin()+(num_output/2), pool1_rst_1.begin());
	vector3D(pool1_rst_2, num_output/2, 27, 27);
	copy(pool1_rst.begin()+(num_output/2),pool1_rst.end(), pool1_rst_2.begin());
	conv_kernel_size = 5, channels  = num_output/2, num_output = 256; //channels 是上一层的num_output
	vector4D(conv2_weights, num_output, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv2_weights_1, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv2_weights_2, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	...
	copy(conv2_weights.begin(),conv2_weights.begin()+(num_output/2), conv2_weights_1.begin());
	copy(conv2_weights.begin()+(num_output/2), conv2_weights.end(), conv2_weights_2.begin());
	copy(conv2_bias.begin(),conv2_bias.begin()+(num_output/2), conv2_bias_1.begin());
	copy(conv2_bias.begin()+(num_output/2), conv2_bias.end(), conv2_bias_2.begin());
	release3D (pool1_rst);
	release (conv2_bias);
	...
	convolution2(pool1_rst_1, stride, pad, conv2_weights_1, conv2_bias_1,  conv2_rst_1, "conv2_1");
	convolution2(pool1_rst_2, stride, pad, conv2_weights_2, conv2_bias_2,  conv2_rst_2, "conv2_2");
	copy(conv2_rst_1.begin(), conv2_rst_1.end(), conv2_rst.begin());
	copy(conv2_rst_2.begin(), conv2_rst_2.end(), conv2_rst.begin()+num_output/2);
	...
	////////////////pool2////////////////
	pooling_kernel_size  = 3; stride  = 2, pad = 0;
	...
	vector4D(conv3_weights, num_output, channels, conv_kernel_size, conv_kernel_size);
	vector <float> conv3_bias(num_output);
	readweights("conv3.bin", conv3_weights, conv3_bias);
	stride = 1, pad = 1; 
	vector3D(conv3_rst, num_output, 13, 13);  
	convolution3(pool2_rst, stride, pad, conv3_weights, conv3_bias,  conv3_rst, "conv3");
	release3D (pool2_rst);
	release (conv3_bias);
	...
	copy(conv3_rst.begin(),conv3_rst.begin()+(num_output/2), conv3_rst_1.begin());
	vector3D(conv3_rst_2, num_output/2, 13, 13);
	copy(conv3_rst.begin()+(num_output/2), conv3_rst.end(), conv3_rst_2.begin());
	conv_kernel_size = 3, channels  = num_output/2, num_output = 384; 
	vector4D(conv4_weights, num_output, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv4_weights_1, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv4_weights_2, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	...
	copy(conv4_weights.begin(),conv4_weights.begin()+(num_output/2), conv4_weights_1.begin());
	copy(conv4_weights.begin()+(num_output/2), conv4_weights.end(), conv4_weights_2.begin());
	copy(conv4_bias.begin(),conv4_bias.begin()+(num_output/2), conv4_bias_1.begin());
	copy(conv4_bias.begin()+(num_output/2), conv4_bias.end(), conv4_bias_2.begin());
	release3D (conv3_rst);
	release (conv4_bias);
	...
	convolution4(conv3_rst_1, stride, pad, conv4_weights_1, conv4_bias_1,  conv4_rst_1, "conv4_1");
	convolution4(conv3_rst_2, stride, pad, conv4_weights_2, conv4_bias_2,  conv4_rst_2, "conv4_2");
	...
	vector4D(conv5_weights, num_output, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv5_weights_1, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	vector4D(conv5_weights_2, num_output/2, channels, conv_kernel_size, conv_kernel_size);
	...
	copy(conv5_weights.begin(),conv5_weights.begin()+(num_output/2), conv5_weights_1.begin());
	copy(conv5_weights.begin()+(num_output/2), conv5_weights.end(), conv5_weights_2.begin());
	copy(conv5_bias.begin(),conv5_bias.begin()+(num_output/2), conv5_bias_1.begin());
	copy(conv5_bias.begin()+(num_output/2), conv5_bias.end(), conv5_bias_2.begin());
	release (conv5_bias);
	...
	convolution5(conv4_rst_1, stride, pad, conv5_weights_1, conv5_bias_1,  conv5_rst_1, "conv5_1");
	convolution5(conv4_rst_2, stride, pad, conv5_weights_2, conv5_bias_2,  conv5_rst_2, "conv5_2");
	copy(conv5_rst_1.begin(), conv5_rst_1.end(), conv5_rst.begin());
	copy(conv5_rst_2.begin(), conv5_rst_2.end(), conv5_rst.begin()+num_output/2);
	...
	softmax(fc8_rst);
	//printf("softmax done\n");
	//print_vec("fc8_softmax_rst.txt", fc8_rst);//fc8_rst是1000个概率数字，对应1000组分类
